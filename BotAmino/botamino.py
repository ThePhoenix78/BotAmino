...
from aminofix.lib.util import Event, UserProfile, gen_deviceId  # type: ignore
# internal
from .bannedwords import BannedWords
from .bot import Bot
from .command import Command
from .parameters import Parameters
from .parser import ParserFeature
from .timeout import TimeOut
from .utils import PATH_CLIENT, safe_exit
from .typing import CallbackCategory
...
class BotAmino(Command, Client, TimeOut, BannedWords):
    ...
    def __init__(
        ...
        Client.__init__(self, deviceId=deviceId, proxies=proxies, certificatePath=certificatePath)  # type: ignore
        ...
        self.communaute: typing.Dict[int, Bot] = {}
    ...
    def tradlist(self, aminoIdOrLink: typing.Union[typing.Iterable[str], str]) -> typing.List[str]:
        """Get a list of user IDs from an iterable of Amino links"""
        uidList: typing.List[str] = []
        for aminoId in [aminoIdOrLink] if isinstance(aminoIdOrLink, str) else aminoIdOrLink:
            ...
            with contextlib.suppress(Exception):
                userId = self.get_from_code(amino_link).objectId  # type: ignore
                uidList.append(userId)  # type: ignore
                continue
            uidList.append(aminoId)  # type: ignore
        return uidList
    ...
    def get_wallet_amount(self) -> int:
        ...
        return self.get_wallet_info().totalCoins or 0  # type: ignore
    ...
    def start_video_chat(self, comId: int, chatId: str, joinType: int = 1) -> None:
        ...
        """
        self.send(json.dumps({  # type: ignore
            "o": {
                "ndcId": comId,
                "threadId": chatId,
                "joinRole": joinType,
    ...
    def check(self, args: Parameters, *can: typing.Literal['admin', 'bot', 'staff'], id_: typing.Optional[str] = None) -> bool:
    ...
    def launch(self, passive: bool = False) -> None:
        ...
        for comId in typing.cast(typing.List[int], amino_list.comId):  # type: ignore
    ...
    def message_analyse(self, data: Event, category: CallbackCategory) -> None:
        ...
        try:
            subClient = self.get_community(typing.cast(int, data.comId))  # type: ignore
        except Exception:
            return
    ...
    def launch_text_message(self) -> None:
        ...
        def text_message(data: Event) -> None:
            ...
            if not self.timed_out(args.authorId) and args.message.startswith(subClient.prefix) and not self.check(args, "bot"):
                subClient.send_message(args.chatId, self.spam_message)  # type: ignore
                return
            # command execution
                ...
                if command in subClient.locked_command:
                    subClient.send_message(args.chatId, self.lock_message)  # type: ignore
                    return
                ...
                # unmatched command
                    ...
                    subClient.send_message(args.chatId, self.no_command_message)  # type: ignore
                return
            # answer execution
    ...
    def launch_removed_message(self) -> None:
        ...
        for type_name in REMOVE_EVENTS:
            @self.event(type_name)  # type: ignore
            def _(data: Event) -> None:
    ...
    def launch_on_member_join_chat(self) -> None:
        ...
        @self.event("on_group_member_join")  # type: ignore
        def _(data: Event) -> None:
    ...
    def launch_on_member_leave_chat(self) -> None:
        ...
        @self.event("on_group_member_leave")  # type: ignore
        def _(data: Event) -> None:
