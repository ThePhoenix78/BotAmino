...
from .typing import Callback, Condition, CallbackCategory, Events, LiteCallback
...
class Command:
    ...
    def __init__(self) -> None:
        self.commands: typing.Dict[CallbackCategory, typing.Dict[str, typing.Union[Callback, LiteCallback]]] = {}
        self.conditions: typing.Dict[CallbackCategory, typing.Dict[str, Condition]] = {}

    def execute(self, name: str, data: Parameters, category: CallbackCategory = "command") -> typing.Any:
    ...
    def commands_list(self) -> typing.List[str]:
        """Get command list names"""
        return list(self.commands["command"])

    def answer_list(self) -> typing.List[str]:
        """Get answer list names"""
        return list(self.commands["answer"])

    def command(
        self,
        name: typing.Optional[typing.Union[str, typing.Iterable[str]]] = None,
        ...
        names = set([name] if isinstance(name, str) else list(name) if isinstance(name, typing.Iterable) else [])
        if callable(condition):
            validate_lite_callback(condition)
        def inner(callback: CallbackT) -> CallbackT:
    ...
    def on_member_join_chat(
        self,
        condition: typing.Optional[Condition] = None
    ) -> typing.Callable[[LiteCallbackT], LiteCallbackT]:
    ...
    def on_event(
        self,
        name: typing.Union[typing.Iterable[Events], Events],
        condition: typing.Optional[Condition] = None
    ) -> typing.Callable[[LiteCallbackT], LiteCallbackT]:
        ...
        names = set(typing.cast(typing.Iterable[str], name if not isinstance(name, str) else [name]))
        if callable(condition):
            validate_lite_callback(condition)
            for key in names:
